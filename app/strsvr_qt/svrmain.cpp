...
void MainForm::SvrStart(void)
{
    ...
    if (Input->currentIndex()==0) {
        if (CmdEna[0]) strncpy(cmd[0],qPrintable(Cmds[0]),1024);
    }
    else if (Input->currentIndex()==1||Input->currentIndex()==3) {
        if (CmdEnaTcp[0]) strncpy(cmd[0],qPrintable(CmdsTcp[0]),1024);
    }
    for (int i=0;i<5;i++) {
        opt[i]=SvrOpt[i];
    }
...
void MainForm::SvrStop(void)
{
    ...
    if (Input->currentIndex()==0) {
        ...
        if (CmdEnaTcp[1]) strncpy(cmd[0],qPrintable(CmdsTcp[1]),1024);
    }
